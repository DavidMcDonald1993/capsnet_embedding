...
class ValidationCallback(Callback):
	...
	def __init__(self, G_neighbours, X, Y, model,
		...
		self.embedding_idx = np.arange(len(G_neighbours))
	...
	def on_epoch_end(self, epoch, logs={}):
		evaluation_logs = self.evaluate(epoch)
	...
	def evaluate(self, epoch):
		...
		predictions = self.model.predict([self.X, self.G_neighbours], batch_size=len(self.X))
		...
		for layer, embedding in enumerate(embeddings): # ?
			...
			logs.update({"mean_rank_reconstruction_layer_{}".format(layer) : mean_rank_reconstruction, 
				"mean_precision_reconstruction_layer_{}".format(layer) : mean_precision_reconstruction,})
			if self.removed_edges_dict is not None:
				...
				logs.update({"mean_rank_link_prediction_layer_{}".format(layer) : mean_rank_reconstruction, 
					"mean_precision_link_prediction_layer_{}".format(layer) : mean_precision_reconstruction,})
			...
			if epoch is not None:
				...
				self.plot_embedding(embedding, 
					mean_rank_reconstruction, mean_precision_reconstruction, distortion,
					path="{}/embedding_epoch_{:04}_layer_{}.png".format(self.args.plot_path, epoch, layer))
	...
	def plot_embedding(self, embedding, 
		...
		plt.suptitle("Dim={} Mean Rank Reconstruction={} MAP Reconstruction={} Distortion={}".format(embedding.shape[1], 
			mean_rank_reconstruction, mean_precision_reconstruction, distortion))
		...
		if self.train_idx is not None:
			train_points = embedding[self.train_idx]
			plt.scatter(train_points[:,0], train_points[:,1], 
				s=10, edgecolors="k", facecolors="none", zorder=2)
	...
	def evaluate_distortion(self, embedding):
		...
		mask = ~ np.eye(len(self.X), dtype=bool)
	...
	def evaluate_class_predictions(self, predictions):
		...
		predictions = predictions[self.val_idx]
