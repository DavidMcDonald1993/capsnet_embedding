...
def load_models(X, Y, model_path, neighbourhood_sample_sizes, num_filters_per_layer, agg_dim_per_layer,
	...
	embedder_lambda = lambda x, l=embedding_layer: connect_layers(zip(layer_dict["agg"][:l], layer_dict["batch_normalization"][:l], 
    layer_dict["cap"][:l], layer_dict["squash"][:l]) + 
    [(layer_dict["agg"][l], layer_dict["batch_normalization"][l], layer_dict["cap"][l]),
	...
	if num_classes > 1 and len(label_prediction_layers) > 0:
		...
		label_prediction_lambda = lambda x, i=len(label_prediction_layers)-1, l=label_prediction_layer: connect_layers(zip(layer_dict["agg"][:l], 
		    layer_dict["batch_normalization"][:l], layer_dict["cap"][:l], layer_dict["squash"][:l]) + 
