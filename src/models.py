...
from graphcaps_layers import AggregateLayer, GraphCapsuleLayer, HyperbolicDistanceLayer, Length, squash, inverse_squash
...
def build_embedder_and_prediction_model(data_dim, num_classes, model, args):
	...
	embedder_input = layers.Input(shape=(embedder_input_num_neighbours, data_dim), name="embedder_input")
...
def generate_graphcaps_model(data_dim, num_classes, args):
	...
	x = layers.Input(shape=(num_neighbours_per_layer[0], data_dim), 
		name="input_layer")
	y = x
	...
	for i, neighbourhood_sample_size, num_primary_caps, num_filters, agg_dim, num_caps, capsule_dim in zip(range(num_layers),
		...
		if num_caps == num_classes:
			label_predictions.append(y)

		layer_embedding = layers.Lambda(inverse_squash, name="embedding_layer_{}".format(i))(y)
